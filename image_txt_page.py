...
from config import LEVEL_ORDER, IMAGE_EXTS


class ImageTxtPage(ttk.Frame):
    def __init__(self, parent, extractor_ref):
        super().__init__(parent)
        self.extractor_ref = extractor_ref
        self.image_folder = tk.StringVar()
        self.address_file = tk.StringVar()
        ...
        self.images_per_folder = tk.StringVar(value="3,4,5")
        self.selection_mode = tk.StringVar(value="顺序选取")
    ...
    def _setup_ui(self):
        ...
        title = ttk.Label(main, text="图文同名 TXT 生成工具 v1.3.0",
                          font=("Microsoft YaHei", 16, "bold"))
        title.pack(pady=(0, 10))

        # 图片文件夹
        row_img = ttk.Frame(main)
        row_img.pack(fill=tk.X, pady=3)
        ttk.Label(row_img, text="图片文件夹：", width=14).pack(side=tk.LEFT)
        ttk.Entry(row_img, textvariable=self.image_folder).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ttk.Button(row_img, text="选择文件夹", command=self._select_image_folder).pack(side=tk.LEFT)

        # 地址 TXT
        row_addr = ttk.Frame(main)
        row_addr.pack(fill=tk.X, pady=3)
        ttk.Label(row_addr, text="地址信息 TXT：", width=14).pack(side=tk.LEFT)
        ttk.Entry(row_addr, textvariable=self.address_file).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ttk.Button(row_addr, text="选择 TXT", command=self._select_address_file).pack(side=tk.LEFT)
        ...
        ttk.Combobox(row4, textvariable=self.extract_mode,
                     values=["优先提取", "强制提取"], state="readonly", width=12).pack(side=tk.LEFT, padx=5)
        ...
        row_out.pack(fill=tk.X, pady=3)
        ttk.Label(row_out, text="输出文件夹：", width=14).pack(side=tk.LEFT)
        ttk.Entry(row_out, textvariable=self.output_folder).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ttk.Button(row_out, text="选择文件夹", command=self._select_output_folder).pack(side=tk.LEFT)

        # 每文件夹图片数 + 图片选取方式
        row_img_cfg = ttk.Frame(main)
        row_img_cfg.pack(fill=tk.X, pady=3)
        ttk.Label(row_img_cfg, text="每文件夹图片数：", width=14).pack(side=tk.LEFT)
        ttk.Entry(row_img_cfg, textvariable=self.images_per_folder, width=16).pack(side=tk.LEFT, padx=5)
        ttk.Label(row_img_cfg, text="逗号分隔，如 2,3,4", foreground="gray").pack(side=tk.LEFT)
        ttk.Label(row_img_cfg, text="图片选取方式：").pack(side=tk.LEFT, padx=(15, 5))
        ttk.Combobox(row_img_cfg, textvariable=self.selection_mode,
                     values=["顺序选取", "随机选取"], state="readonly", width=12).pack(side=tk.LEFT)
        ttk.Label(row_img_cfg, text="顺序=按文件名顺序分配；随机=各文件夹独立随机",
        ...
        # 日志
        self.log_area = scrolledtext.ScrolledText(main, height=12, width=80, state=tk.DISABLED)
    ...
    def _select_image_folder(self):
        ...
        if folder:
            ...
            self._show_image_summary(folder)
            if self.smart_mode.get():
                self._analyze_addresses(folder)

    def _select_address_file(self):
        file = filedialog.askopenfilename(
            title="选择地址信息 TXT",
            filetypes=[("文本文件", "*.txt"), ("所有文件", "*.*")],
        )
        if file:
            self.address_file.set(file)
            self._show_address_summary(file)

    def _show_address_summary(self, file):
        """选择地址文件后显示地址统计和前几行预览"""
        try:
            content = None
            ...
            if count > 5:
                preview_str += "..."
            self._log(f"地址文件包含 {count} 个地址，前几项：{preview_str}")
        except Exception:
            pass

    def _show_image_summary(self, folder):
        """选择图片文件夹后显示图片统计"""
        try:
            images = []
            ...
            count = len(images)
            if count > 0:
                self._log(f"图片文件夹包含 {count} 张图片")
            else:
                self._log("图片文件夹中没有支持的图片文件")
        except Exception:
            pass
    ...
    def _analyze_addresses(self, folder):
        """智能分析：用图片文件夹名作为地区样本"""
        import os
        ...
        try:
            candidates = extractor.get_all_candidates(Path(folder).name)
            if candidates:
                lowest = max(c["level"] for c in candidates)
                lowest_levels[lowest] += 1
        except Exception:
            return
        if not lowest_levels:
            return
        most_common = lowest_levels.most_common(1)[0][0]
        level_name_map = {1: "省级", 2: "市级", 3: "区县级", 4: "乡镇街道级", 5: "村社区级"}
        self.target_level.set(level_name_map.get(most_common, "区县级"))
    ...
    def _start_generation(self):
        ...
        address_file = self.address_file.get().strip()
        output_folder = self.output_folder.get().strip()
        keyword_file_path = self.keyword_file.get().strip()
        keywords_text = self.keywords_text.get("1.0", tk.END)

        if not image_folder or not os.path.isdir(image_folder):
            messagebox.showwarning("提示", "请先选择有效的图片文件夹")
            return
        if not address_file or not os.path.isfile(address_file):
            messagebox.showwarning("提示", "请先选择有效的地址信息 TXT 文件")
            return
        ...
        has_kw_file = keyword_file_path and os.path.isfile(keyword_file_path)
        has_kw_text = bool(keywords_text.strip())
        if not has_kw_file and not has_kw_text:
        ...
        raw = self.images_per_folder.get().strip()
        images_per_folder = []
        for part in raw.split(","):
            ...
            try:
                n = int(part)
                if n < 1:
                    messagebox.showwarning("提示", "每文件夹图片数至少为 1")
                    return
                images_per_folder.append(n)
            except ValueError:
                messagebox.showwarning("提示", f"无效的每文件夹图片数：{part}")
                return
        if not images_per_folder:
            messagebox.showwarning("提示", "请填写每文件夹图片数（逗号分隔）")
            return
        ...
        if required_keywords_text.strip():
            ...
            if req_lines:
                total_req_len = sum(len(l) for l in req_lines) + len(req_lines) - 1
        ...
        def run():
            try:
                from image_txt_worker import run_image_generation
                extractor = self.extractor_ref.get("extractor")
                ...
                sel_mode = "sequential" if self.selection_mode.get() == "顺序选取" else "random"

                result = run_image_generation(
                    image_folder=image_folder,
                    address_file=address_file,
                    output_folder=output_folder,
                    keyword_file=keyword_file_path,
    ...
    def _show_region_mapping(self, region_origin_map):
        ...
        columns = ("序号", "可信度", "提取地址", "地址数") + tuple("原地址%d" % (i+1) for i in range(max_cols))
        ...
        tree.heading("地址数", text="地址数", anchor=tk.CENTER)
        ...
        tree.column("地址数", width=55, anchor=tk.CENTER, stretch=False)
        for i in range(max_cols):
    ...
    def _save_mapping(self, tree, max_cols):
        """保存映射表格为 Excel 文件"""
        folder_name = Path(self.output_folder.get()).name if self.output_folder.get() else "地址映射"
        ...
        try:
            ...
            headers = ["序号", "可信度", "提取地址", "地址数"] + ["原地址%d" % (i+1) for i in range(max_cols)]
